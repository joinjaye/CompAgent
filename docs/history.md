# compAgent 历史记录（Phase 完成情况 / 真实验收日志归档）

本文件是 `CLAUDE.md` 的历史归档：每个 Phase 的完整实现细节、真实网络验收记录、
踩过的坑和修复过程，按时间顺序原样保留在这里。`CLAUDE.md` 只保留当前状态速览和
指向本文件的指针，不再重复这些叙述。

**这是历史日志，不是当前状态文档**——读的时候注意甄别：某个"已知限制"/"待处理"
可能在更晚的小节里已经被解决了（`CLAUDE.md`"当前状态与已知问题"一节是最新准确的
状态来源，本文件仅供追溯"为什么会是现在这样"）。
...
## Dashboard 规则透明化与 UI/UX 收口（2026-07-21）

根据新版 Competitor Intelligence Dashboard 业务定义，对既有 category-first 看板
做了规则和体验收口；未改变 SQLite 唯一真相源、静态 JSON 导出、GitHub Pages 部署
架构。

- 安装并使用 `ui-ux-pro-max`，将设计系统持久化到
  `design-system/competitor-intelligence-dashboard/MASTER.md`。采用 Data-Dense
  Dashboard、低动效、高信息密度、键盘焦点、reduced-motion 和 375px 响应式规则。
- Overview Highlights 改为严格的确定性规则：模块/事件类型优先级 → 发布时间倒序，
  全局最多 5 条、每家交易所最多 2 条；不再使用 LLM priority/diff_type 排序。
- Campaign/Product/Listing 与地区摘要统一改为 Rule-based 排序。Campaign 为
  New → Updated → Ongoing → Upcoming → Ended；Product 为 New → Updated；Listing
  为 New Spot → New Perpetual → Delisting → 未识别，同级按发布时间倒序。
- Campaign/Product 默认展示 Top 10，并提供 Load More；完整历史仍全部下发到 Search，
  Search 支持精确 From/To 日期、既有筛选/分页/CSV。Overview 日期选择非最新日期时，
  自动跳转 Search 并带入精确日期，不再只是无效跳转。
- 前端新增每页可展开的“口径与排序规则”。Zoomex 标签改为可核验语义：
  “已有同类 / 已有同类·机制不同 / 未检索到同类 / 需人工核验 / 未进行对比”；明确
  “未检索到”不是确认缺失，“未进行对比”不是业务上的“不适用”。
- 生产库检查：399 条竞品公告 URL 全部非空；主要明细、Listing、Markets、Search
  均提供原始公告链接。`docs/data/dashboard.json` 已用主库重新导出。

验证：Dashboard `18 passed`，全量 `385 passed`；真实 Chromium 检查 6 个一级 tab、
Search 分页和 375px 视口，console/page error 为 0，页面级横向溢出为 0。

## Dashboard v2：事件级合并、Product 对比修复与视觉重构（2026-07-21）

用户复核上一版后指出视觉拥挤、跨语言仍逐行展示、Product 全部“未进行对比”，并要求
重新审计所有占位能力。本轮以生产库为准完成修复：

- `ui-ux-pro-max` 重新生成 page-level dashboard 设计方案；页面从 1120px 暗色紧凑布局
  改为最高 1480px 的深海军蓝运营控制台（用户最终确认保持深色主题）。导航、卡片、筛选、表格、状态与对比证据建立独立
  层级；宽表使用容器内滚动，不再扩张页面；375px 页面宽度无溢出。
- 导出层新增 `_merge_localized_rows()`：按 `source + group_id` 合并同一业务事件；对
  Phemex 等跨语言 group_id 不可靠的情况，仅在跨 locale 且规范化标题一致时兜底合并，
  不合并同 locale 的同名事件。每行保留 `markets[]` 和 `localized_variants[]`，前端并列
  展示市场 chip 与各语言原文链接。Campaign/Product/Listing/Search 均使用事件粒度。
- Product 根因确认：2026-07-20 的 10 个去重事件虽都有 v3 分析，但逐条
  `evidence_indices` 全为空，校验层按防幻觉规则强制 `diff_type=不适用`；结构化
  `zmx_baseline` 仅 19 条 locale 记录，且原生产分析只按同 locale、近 90 天召回，覆盖
  不足。前端此前把这一真实缺口压扁为“未进行对比”，没有说明原因。
- 新增全量 Zoomex Product 公告基线的确定性召回：先按 Product Coverage taxonomy
  对齐，再做标题高权重词项匹配，过滤通用词并要求强证据；只输出候选和源链接，不把
  “未匹配”断言为缺失。当前 10 个 Product 事件：6 个找到相关基线候选、4 个未匹配、
  0 个已有带引用的最终对比结论。页面显式展示三档状态与定义。
- Product 新增确定性 `product_category` 与 `update_kind`；Coverage 以事件粒度展示，
  Update 类型支持 New Product / Feature Added / Feature Removed / Rule Updated /
  UI Updated / Performance Updated / Operational Notice。
- Campaign 新增 `campaign_all`，页面不再只展示最新批次变化；Summary 同时呈现 New、
  Updated、Ongoing、Ending Soon。无法从公告/insight 获得时间的事件显示 Date unknown，
  不伪造生命周期。
- 修正 Trend 口径：从只统计 `new` 改为 `new + changed`，继续用 group 粒度去重并排除
  unchanged 重复抓取。
- Overview 新增 Data Readiness，公开三项未完成的数据能力：Campaign 时间字段覆盖、
  Product Entity/Change History、KR collector。全局 Exchange/Market 筛选会作用于各详情
  页面与 Overview 汇总。

验证：Dashboard `20 passed`；全量 `395 passed`。真实 Chromium 检查 6 个一级 tab、
全局 FR 筛选（Product 10→5）、375px 控件与页面宽度；page error 为 0、页面级横向
溢出为 0。生产 JSON 已重新导出。

## 政策确认：非 Zoomex 源永不回填主库，只做每日增量（2026-07-21）

用户明确拍板一条长期运营政策，适用于全部竞品源（Bitunix/Weex/BingX/Phemex/Lbank）：
**除 Zoomex（我方基线，需要定期全量核查兜底，见「Zoomex daily 增量改分页数上限 +
全量建仓」一节）外，其余任何源都不再对主库 `data/competitor_intel.db` 做历史回填**，
之后对这些源的使用只是「每天运行一次，访问并存储当天的增量」。

- **这是记录在案的政策，本次 session 未据此在主库上执行任何采集动作**（用户明确
  选择"先只记录，暂不执行"）——`data/competitor_intel.db` 里 Weex 目前仍是 0 行
  （见「Weex 路径问题」一节），本次未触碰。
- **对现有机制的影响**：`--lookback-days` 参数（见「`--lookback-days` 修复」一节）
  本来就是为了防止 daily 增量退化成全量回填而加的，这条政策等于确认了它的使用
  方式——以后任何一次对非 Zoomex 源的采集，都应该带上合理的 `--lookback-days`
  （或者确保 `crawl_state`/watermark 已经是从某次真实增量运行开始的连续状态），
  不能再像更早几次 session 那样为了"看看能采到多少"直接对空库跑全量。
- **对 Weex 的直接含义**：下次真正启用 Weex 时（「Weex 路径问题」解决之后），
  不需要、也不应该把 Phase 2 时代那种"全平台历史一次性建仓"的动作在主库上重放
  一遍——直接从当天开始做每日增量即可，主库里 Weex 的历史深度会比 Zoomex 浅得多，
  这是政策决定的结果，不是遗漏。
- **未决问题**：这条政策没有回答"多少天的 lookback 才算合理的每日调度默认值"
  （`max_pages`/`--lookback-days` 目前都是经验值，未做过真实数据校准，见
  「daily 增量分页上限 5→2」一节的已知限制），后续接入 Phase 8 调度时需要定一个
  具体的默认参数，而不是每次手动指定。

## Phemex 跨语言归组修复：group_id 改用 URL slug（2026-07-21）

「Phemex 跨语言归组问题」（多处历史 session 记录：EN/FR 的 `article_id` 不同导致
`group_id` 拼不到一起，产生 region-exclusive 假阳性、阻断 EN→FR 分析复用，此前
只在 Dashboard 聚合层用"group_id + 规范化标题"兜底）本次真正定位并修复了根因，
不再只是展示层兜底。

### 真实核实（先查，不猜）

对主库当前全部 68 条 Phemex 行（EN 34 + FR 34）按 `url` 分组：**100% 两两配对，
0 条落单**——同一篇公告 EN/FR 的 `url`（对应 `parse_query_response()` 返回的
`slug` 字段，来自 `prod-cms-api.phemex.com/articles/query` 真实分页 API）逐字节
相同，只有数值 `article_id`（`id` 字段）不同。例：`KORUUSDT Futures` 下架公告
EN `article_id=135479` / FR `article_id=135481`，两者 `url` 均是
`https://phemex.com/announcements/phemex-will-delist-koruusdt914`。覆盖
campaign/listing/delisting/other 全部已入库 category，无一例外，且全部 68 行
`url` 非空——`slug` 是源站自己生成、天然跨 locale 一致的标识符，不是靠标题/时间
相似度做模糊匹配就能达到的可靠度。

### 修复

`src/collectors/phemex.py` 新增 `_slug_from_url(url)`（从
`.../announcements/<slug>` 提取 `<slug>`，剥掉 query/fragment），
`normalize()` 里 `group_id` 从 `f"phemex_{article_id}"` 改成
`f"phemex_{slug}"`（`slug` 取不到时退回 `article_id` 并记警告，理论保底，未在
真实数据中触发过）。`tests/collectors/test_phemex_collector.py` 新增 5 个用例
（slug 提取的正常/带 query fragment/取不到三种情况 + 跨 locale 归组结果验证 +
无 url 时的兜底验证），原有 `test_normalize_uses_category_key_not_response_field`
的 group_id 断言同步更新。`pytest` 全量 390 通过。

### 主库数据修复（重算派生列，不是重新采集，不受「非 Zoomex 源永不回填」政策约束）

对已存量的 68 条 Phemex 行，用同一份 `_slug_from_url` 逻辑重算 `group_id`
（`UPDATE announcements SET group_id=... WHERE uid=...`，不发任何网络请求，只是
用已经落库的 `url` 字段重新计算一个派生列），执行前按惯例先备份整库
（`data/competitor_intel.db.bak_20260721_103943_phemex_groupid_fix`）：

```
68 行全部需要更新（旧 group_id 全部是按 article_id 拼的，逐一核对无遗漏）
distinct group_id：68 -> 34（完全两两配对，验证了上面"100%配对"的真实核查结论）

python -m src.pipeline group-check --sources Bitunix,BingX,Phemex,Lbank,Zoomex
# 检查了 998 个 group，0 异常

python -m src.pipeline region --sources Phemex
# 共 34 个 group，0 个判定为地区独占（修复前 34 个 EN/FR 对全部被误判为独占，
# 修复后清零，这正是历史 session 反复怀疑的"假阳性"）
```

### 已知限制 / 未处理（如实记录）

- **`insights` 表里已有的 4 条 Phemex 批次分析（campaign EN/FR、delisting EN/FR）
  未重新计算**：这些是在旧 group_id 下产出的，`Phemex/campaign/FR` 当时是独立
  调用 LLM（未走 EN 复用，`llm_tokens_used=20125`），理论上 group_id 修复后
  `can_derive_from_en()` 可能会判定为可复用，但重新验证这一点需要真正执行一次
  `python -m src.analysis --source Phemex`（即使大概率走缓存/复用不产生新的真实
  token 消耗，也会改写 insights 表内容）——按「cap real LLM calls during
  verification」的一贯做法，这次没有未经确认就顺手跑掉，留给用户决定要不要刷新。
  `delisting` 类目的两条本来就是 Phase 4 v3 政策收紧（只分析 campaign/product）
  之前的历史遗留数据，Dashboard 导出层已经在忽略它们，不受这次 group_id 修复
  影响，不需要处理。
- **`_slug_from_url` 的 fallback 路径（url 取不到 slug 时退回 article_id）在真实
  数据上从未触发过**（68/68 行全部命中 slug），只是代码防御，未来如果 Phemex
  改版导致某条 url 格式变化，这条路径会静默退化回旧行为（记警告日志，不会崩），
  需要留意日志里"group_id 退回 article_id"这行。
- **只验证/修复了 Phemex**：跟 Phemex 类似"article_id 跨 locale 不一致"的问题
  未在其它源上专门核查过（Bitunix/Weex/Zoomex/Lbank 的 group_id 设计前提是
  article_id 跨 locale 一致，历史 session 各自用真实数据验证过这个前提在各自源
  成立，BingX 也是同样前提且已验证成立，见 Phase 2.7"group_id 跨语言一致性验证"
  一节）。

## Zoomex Capability Catalog + staged.py 竞品分析管线 + Dashboard 事件级 Detail（2026-07-21）

用户提出一份完整的五阶段实现 plan（Phase①-⑤），核心动机：现有 Zoomex 对比机制有两个
根本缺陷——`zmx_baseline.mechanism_type` 是 LLM 自由生成的中文标签，同一玩法反复起
近义新名字（63 个真实标签里能看出大量重复）；`list_pending_zoomex_rows()`/
`get_baseline_digest()` 只看近 90 天窗口，系统结构性地无法断言"Zoomex 真的没有这个
能力"，只能说"最近没看到"。规划阶段先花了大量时间做只读探索（3 个并行 Explore agent
+ 1 个 Plan agent），确认了几个会影响实现范围的真实情况：`docs/index.html`/
`export_data.py` 当天早些时候已经有一次未提交的"Dashboard v2"改动（事件级合并、
规则排序、product-only 的词项重叠匹配 stopgap）；`src/analysis/staged.py`（两段式
事实抽取+业务判断）早就写好且有完整测试，但从未接入 `run.py`；`zmx_baseline.py`
当天也有一个更小的局部修复在制品（locale-scoping bug）。跟用户逐一确认后，五个
Phase 按顺序全部实现完成，全程真实网络请求验证（含真实 cursor_agent 调用），未
使用任何合成/伪造数据。

### Phase①：Zoomex Capability Catalog（取代 zmx_baseline 全部机制）

`src/analysis/zmx_baseline.py` 整个删除，替换为 `src/analysis/zmx_catalog.py` +
新表 `zmx_summary`（per-article 结构化提取）/`zmx_catalog_entry`（rollup，纯 SQL
聚合，不调用 LLM）。两个根本修复：
1. `mechanism_type` 从自由生成文本改为 `config/zmx_mechanism_taxonomy.yaml` 定义的
   封闭/半封闭枚举——campaign 的 ~18 个 key 是对主库 `zmx_baseline` 表里真实存在的
   63 个碎片化标签做的一次人工归并（每个 key 附真实标题做 examples，不是凭空设计）；
   product 直接复用 `export_data.py` 已有的 11 个标签（Copy Trading/Card & Payment/
   Security/Earn/API/Bot/Wallet/Convert/Institutional/Trading/Others），保证跟
   看板既有展示逻辑用同一份分类。不匹配任何 key 时落 `other` + `raw_mechanism_label`
   保留原始描述，不允许 LLM 自造新类型。
2. 提取/查询都不再有 lookback 窗口，覆盖 Zoomex 全量历史——`get_catalog_digest()`
   沿用旧版"类型覆盖优先于单类型深度"的采样算法（这一层价值不受数据源变化影响，
   原样保留），只是数据源换成 `zmx_summary` 且去掉了 90 天过滤。
3. EN→locale 复用改成**逐条**判断（`find_derivable_en_summary`，按 group_id 找
   同一事件的 EN/EN-Asia 版本），比旧版整批复用的粒度更细，也更简单（不需要
   `batch.can_derive_from_en()` 那种"整批 group_id 集合完全相同"的保守校验）。

真实验证：`rollup` 对主库真实数据跑通（18 个非-other campaign key 全部产出，
`exists_flag` 分布合理）；真实 `extract`（cursor_agent，$0.055/15 条）产出的
`core_summary`/`reward_amount`/`mechanism_type` 全部具体可信、无编造。

CLI 从 `python -m src.analysis.zmx_baseline` 改为 `python -m src.analysis.zmx_catalog
{extract,rollup,all}`；`scripts/run_daily_pre_lark.sh` 同步更新调用方式。踩到一个
真实 argparse 坑：`--db` 如果同时注册在顶层 parser 和子命令 parser 上，子命令 parser
的 default 会在解析时无条件覆盖掉顶层已经解析出的值（两个 parser 共享同一个 dest
时的经典陷阱），`--db X rollup` 这种"放在子命令前"的写法会静默退回默认库、不报错——
修复为只在子命令 parser 上注册 `--db`，强制必须写在子命令名之后，这样出错时是清晰
的 usage error 而不是静默用错库。`scripts/consolidate_zmx_labels.py`（当天早些时候
另一个 session 的在制品，局部修复 zmx_baseline 的 locale-scoping bug）随
`zmx_baseline.py` 一起删除——不需要了，问题从根上被封闭枚举解决。

### Phase②：staged.py 接入 run.py，删除单体 v1/v2/v3 prompt

`run.py` 从"一次 LLM 调用产出整批分析"改成三段：Stage1（每篇公告独立事实抽取，
`extraction_cache_key` 按 content_hash 缓存）→ Stage2（无 LLM，`recall_candidates()`
确定性词项召回）→ Stage3（一次批量业务判断调用，只读 Stage1 事实 + Stage2 候选，
不重新读公告原文）。`priority` 完全由 `calculate_priority()` 程序计算，AI 不再直接
产出；`action_type`/`owner`/`follow_up` 也从 LLM 输出里整体移除（改由 Phase⑤ 规则
派生）。

关键发现：`staged.py` 的 `gap_type` 枚举
（confirmed_gap/baseline_not_found/different_mechanism/covered/not_applicable）
几乎正是想要的 5 值 diff_type，只是英文——决定**保留 gap_type 作为内部/prompt
wire 格式，只在写库前这一个边界（`llm.py` 的 `GAP_TYPE_TO_DIFF_TYPE`）翻译成中文**，
不是简单改名：`baseline_not_found`（没召回到候选，不能断言缺失）必须映射到「不适用」，
不是「ZMX缺失」，这是防止误报缺失的核心区分；`混合` 不再是 LLM 需要理解的概念，
改成对批次内各条 diff_type 的纯程序聚合（`aggregate_batch_diff_type`）。

`llm.py` 删除 `validate_and_normalize`/`_normalize_article_fields`/`AnalysisResult`/
全部 `_VALID_*` 枚举表，新增 `validate_fact_extraction`/`validate_business_judgment`
（逐条校验，唯一会丢弃整条的规则是 index 不在批次内；候选为空但断言了实质性差异会
程序性降级为 `baseline_not_found`，防幻觉）。`compute_cache_key()` 意外地不能删——
`zmx_catalog.py`（Phase①）还在用它做提取缓存，这是当初写 Phase②计划时没预料到的
真实依赖，发现后原样保留。`prompts.py` 删除全部 v1/v2/v3 单体模板（`SYSTEM_CAMPAIGN`/
`SYSTEM_CAMPAIGN_V3`/`build_prompt`/`build_zmx_block` 等），只保留 staged-v1 两段
prompt + zmx-catalog-extract-v1 + daily-digest-v1。

真实验证（scratch db，cursor_agent，Bitunix 2026-07-15 campaign+product）：4 次真实
调用（Stage1×2 + Stage3×2），EN→FR/ID 正确零成本复用，产出的逐条 `event_type`/
`mechanism`/`diff_type`/`diff_detail` 字段形状和内容均正确，`baseline_not_found`→
`不适用` 的防误报逻辑在小样本目录下按预期触发（没有强行编造"缺失"结论）。

### Phase③：`export_data.py` 对照字段重构

`_load_article_index()` 改读新字段名（`mechanism`/`feature` 取代
`mechanics`/`feature_description`，`start_at`/`end_at` 取代单一 `time_window`
字符串），保留 `.get()` 优雅降级（老数据/老字段名取不到就是 None，不抛异常）。
新增 `_load_zmx_catalog_index()`/`_load_zmx_counterpart_index()`，`zmx_exists`/
`zmx_mechanism_type`/`zmx_capability_desc`/`zmx_counterpart` 现在**campaign 和
product 都有**（不再是只有 product 的特殊字段）。

彻底退休当天早些时候"Dashboard v2"引入的 `_load_product_baseline()`/
`_product_baseline_candidates()`（基于全量历史词项重叠的确定性匹配 stopgap，
`comparison_status in (candidate_found, baseline_unmatched)`/`zmx_candidates`）——
那套机制是在真实 LLM 管线覆盖不到 campaign、product 基线还有 90 天窗口限制时的
权宜之计，Phase①+②上线后继续保留会导致同一行公告出现两套互相矛盾的结论。
`comparison_status` 简化为 `analyzed`（有真实 Stage1/Stage3 结果，含"不适用"）/
`pending`（还没被任何分析运行覆盖，不是"确认无差异"）两态。

真实验证：对主库跑一次真实导出，确认老 insights（Phase②之前产出、字段名不同）
优雅降级为空对照，不报错；`quality.product_comparison` 正确统计
`analyzed=37 pending=0`（该库当时所有 product 行都至少被某次分析覆盖过，即使是
老形状）。

### Phase④：Campaign/Product Detail 抽屉式展开 + 筛选器

确认 `docs/index.html` 里 `detailCard()` 函数确实是死代码（定义了但从未被调用），
在这个基础上新增真正的点击展开机制（`row-expand-toggle` 按钮 + `st.expanded`
Set 记录展开状态，重渲染时保留），展开后显示 Basic Info / Rule-Reward-Timeline
（或 Feature）/ AI Summary / Zoomex Comparison 四列，Zoomex Comparison 是竞品 vs
Zoomex 对照示例的两栏布局。同时补齐 Type/Status/Diff/Keyword 筛选（Product 另加
Market），下拉选项从当前数据动态生成，不是写死列表。

修复 Phase③退休 `comparison_status`/`zmx_candidates` 后遗留的真实断裂点：
`productComparison()`（读旧字段）、`chipCard()` 的 product 专属统计、
`renderCategoryPane()` 的 product 专属 stat 卡片、quality-banner 文案，四处
全部改用新字段，重命名为 `zmxComparisonBox()`（campaign/product 通用）。

真实验证：真实 cursor_agent 分析产出的数据 + Playwright 无头浏览器，6 个顶层
tab 全部 0 console/page error，Detail 展开/收起、新筛选器（含下拉选择差异类型）
均可正常交互，375px 视口无横向溢出。

### Phase⑤：AI Insight 复活 + Follow-up 规则化

`src/analysis/daily_digest.py` 新增 `provider` 参数支持 `cursor_agent`（原来只支持
`openai_http`，但 `.env` 只配置了 Cursor 凭证，不加这个改动就没法做真实验证）。
`export_data.py` 新增 `build_daily_digest()`：只读 `peek_cached_digest()`（从不
触发 LLM 调用），命中真实缓存则 `source="llm"`，否则 `source="fallback"`。因为
新版 Overview 是跨 locale 的单一入口，不再有逐 locale 的位置，固定取 EN
locale（每个竞品都覆盖的市场）代表全局。前端加 `todaysInsightMarkup()` + 
`.tag-llm` 徽标，明确区分"LLM 生成"和"占位符 · 待接入 LLM 生成"，不让占位文案
冒充真实结论。

`_derive_follow_up()`（新增，纯规则，不调用 LLM）：`diff_type=ZMX缺失` 且该
`mechanism_type` 本批被 ≥2 个不同竞品触及（行业共性信号，不是单一竞品孤例）→
"建议评估跟进"；`ZMX玩法不同`→"建议观察差异"；`ZMX已有`→"无需关注"；其余不产出。

真实验证：对同一个 scratch db（含真实 Phase②分析产出）分两次真实调用
`generate_daily_digest`（第一次用错了日期——`as_of_date` 是全库 `MAX(fetched_at)`
而不是分析时指定的 batch_date，二者在多日混合数据的库里可能不同，重新对准确日期
调用后成功）：产出的当日综述真实读取了 BingX/Bitunix/LBank/Weex/Phemex 跨源当日
批次，归纳出"下架集中清理、上币主攻 TradFi"这类具体、可信、有具体标的引用的
结论（不是空话），`priority_focus` 也给出了具体可执行的建议，前端 `.tag-llm`
徽标真实渲染验证通过。

### 遗留（如实记录）

- **主库 `data/competitor_intel.db` 本次全程未被真实改动**：所有真实 LLM 验证都在
  `/tmp` 下的 scratch db 副本上进行（每次用完即删），主库目前仍是旧的
  `zmx_baseline` 表结构，没有 `zmx_summary`/`zmx_catalog_entry`。下次要在生产上
  用新管线，需要：`python -m src.db init`（补新表，不影响旧数据）→ 决定是否
  `DROP TABLE zmx_baseline`（新旧机制不共存也不冲突，但旧表数据不会被新管线读取，
  留着只是历史痕迹）→ 跑一次真实 `python -m src.analysis.zmx_catalog all` 建仓 →
  重新跑 `python -m src.analysis` 让存量 insights 换成新字段形状。
- **`config/zmx_mechanism_taxonomy.yaml` 的 campaign 枚举只用真实数据校准过一轮**：
  18 个 key 是从当天存量 63 个碎片标签归并出来的，后续如果出现明显不属于任何
  已有 key 的新玩法（真实数据会持续落到 `other` + `raw_mechanism_label`），需要
  人工回头补枚举，不会自动扩展。
- **daily_digest 的 locale 固定取 EN**：如果以后想要更细粒度（比如 FR/VN/ID 各自的
  当日综述），需要决定这些放在看板哪个入口——新版 Overview 是全局单一位置，没有
  逐 locale 的展示位。
...
## Zoomex 详情页 URL 补全：Detail 抽屉里 Zoomex 对照卡片终于可点击（2026-07-22）

用户反馈 Detail 抽屉里 Campaign/Product 的「Zoomex Comparison」右侧卡片标题不是
链接，点不进去看 Zoomex 原文，跟左侧 Weex/竞品那栏的「Open source ↗」不对称。

### 根因排查（先查库，不是前端 bug）

`docs/index.html` 的 `zmxCompareTwoColumn()` 其实早就写好了 `c.url ? <a>...` 的
分支，前端逻辑没问题。查 `docs/data/dashboard.json` 对应记录发现
`zmx_counterpart.url` 是 `null`；再查主库 `announcements` 表：**全部 2054 行
Zoomex 数据 `url` 列 100% 为 NULL**。根因在 `src/collectors/zoomex.py::normalize()`
——一直硬编码 `url=None`，代码里的注释写着"Zoomex 详情页 URL 规则只在 EN 确认过
一个真实样例（`help.zoomex.com/en/article/3858`），其它 locale 的 path segment
没有逐个验证过，按『不允许猜测数据』的约束先留空"。也就是说这不是遗漏，是当时
故意为之的保守选择，一直没有人回来补验证。

### 真实核实（Playwright headless browser，不是 curl——SPA 纯客户端渲染）

1. `https://www.zoomex.com/sitemap-local.xml` 列出全站合法 locale 前缀
   （`en`/`fr-FR`/`id-ID`/`vi-VN`/`en-AS`/`de-DE`/... 等 14 个）。
2. 用 Playwright 打开 `https://help.zoomex.com/{locale}/article/4115`
   （EN/FR/ID/VN/EN-Asia 各一次），观察到该域名会做**客户端路由重定向**到
   `https://www.zoomex.com/{locale}/help/article/4115`（不是服务端 301，纯 curl
   只能拿到通用 SPA 壳，看不出重定向，这也是本项目 Zoomex 采集从 Phase 1 起就
   全程靠 headless browser 侦察的原因）。
3. 5 个 locale 全部渲染出跟主库对应 `announcements.title` 逐字匹配的正文
   （截图核对，如 FR 渲染出`Protection de la Première Transaction｜Recevez
   jusqu'à $100 de Bonus`，与 DB 里该 uid 的 title 完全一致）。
4. 确认 EN 的 URL path segment 是 `en`，不是 API 请求体里用的 `lang_code`
   （`en-US`）——两者是两套独立的值，FR/EN-Asia/VN/ID 这四个则恰好
   path segment 与 `lang_code` 相同（`fr-FR`/`en-AS`/`vi-VN`/`id-ID`）。

### 修复

- `config/sources.yaml` 五个 Zoomex locale 块（EN/FR/EN-Asia/VN/ID）各新增
  `url_locale` 字段，带注释记录验证方式和验证日期，不是拍脑袋填的。
- `src/collectors/zoomex.py::normalize()` 改成
  `url=f"https://www.zoomex.com/{self.config['url_locale']}/help/article/{article_id}"`
  （直接存重定向后的规范 URL，省一次跳转）。
- `tests/collectors/test_zoomex_collector.py`：测试配置补 `url_locale: "en"`，
  原先断言 `ann.url is None` 的用例改成断言具体 URL（改名
  `test_normalize_builds_group_id_and_url_and_leaves_category_none`）。
- 全量 `pytest` 407 passed。

### 主库数据修复（重算派生列，不是重新采集，不受「非 Zoomex 源永不回填」政策约束）

对存量 2054 条 Zoomex 行按 locale 分组用同一份映射批量 `UPDATE
announcements SET url = ...`（只读已落库的 `article_id` 拼字符串，不发任何网络
请求），执行前先备份整库到 scratchpad。EN 703 / FR 364 / EN-Asia 325 / VN 542 /
ID 120，共 2054 行，更新后 0 行 `url` 仍为 NULL。重新跑
`python -m src.dashboard --db-path data/competitor_intel.db --out
docs/data/dashboard.json` 导出，Playwright 真实验证 Detail 抽屉「Zoomex
Comparison」卡片标题现在是可点击链接，指向验证过的真实 Zoomex 文章页。

### 已知限制（如实记录）

- `url_locale` 只对项目范围内用到的 5 个 locale（EN/FR/EN-Asia/VN/ID）做了验证，
  其余 sitemap 里列出的 locale（de-DE/es-MX/ja-JP/ko-KR/pt-BR/ru-RU/th-TH/
  zh-MY/zh-TW）未验证，也不在本项目采集范围内，不需要处理。
- 未重新触发任何 LLM 分析——这次改动只影响 `announcements.url` 这一个展示字段，
  不影响 `insights`/`zmx_summary` 里已经算好的分析结论，`docs/data/dashboard.json`
  已重新导出但 batch_date 等其它内容未变。

## Dashboard 清理占位/精度过高的展示内容（2026-07-22）

用户反馈两处不该在前端展示：Overview 底部「Data readiness」三张卡片（自我表态式的
"尚未伪装成已完成的能力"文案）、Product 顶部「Comparison coverage」告警条。另外
Detail 面板、Campaign Timeline、Campaign Update、Daily Highlights 等处一律显示带
时分秒的完整 ISO 时间戳（如 `2026-07-12T16:00:00Z`），看着乱，要求只保留日期；并
在顶部标注看板整体时间基准是 UTC 还是 +8。

- `docs/index.html`：删掉 `renderOverviewPane()` 里的 Data readiness 区块（连同
  `.readiness-grid`/`.readiness-item` 用途的说明性文案一起删，CSS 类保留未删——
  未来若重新引入类似卡片可以复用）、`renderCategoryPane('product')` 里的
  `Comparison coverage` `.quality-banner`。
- 所有直接把 `post_time`/`start_date`/`end_date`/`launch_time`/`generated_at` 拼进
  `esc()` 的地方统一改成先过 `formatDate()`（`YYYY-MM-DD`），涉及 Detail 面板
  Start/End/Announcement date/Launch date、Campaign List Timeline 列、Campaign
  Update 表、Listing Launch Date 列、Overview Daily Highlights 表；对应表头从
  "...Time" 改名 "...Date"。前端 `formatTime()`（返回 `HH:MM`）已删除，不再有调用方。
- **真正的根因在 `src/dashboard/export_data.py`**：Overview 默认视图（无自定义日期/
  平台筛选时）走的是导出时预计算好的 `DATA.overview.highlights`，其中 `time` 字段
  由 Python 端 `_format_time()` 生成，本来就是 `HH:MM`——只改前端 JS 不会影响这条
  路径（有筛选时前端会重新计算一份 `ov.highlights`，那条路径改了才生效，两条路径
  必须一起改，否则默认视图看起来像没生效）。`_format_time()` 改名
  `_format_date()`，直接返回 `iso_ts[:10]`，替代原来的 split-on-"T" 时分实现。
  `tests/dashboard` 29 passed（原测试未对 `time` 字段格式做断言，未受影响）。
- 顶部 `.topbar-left` 新增 `时间基准 · UTC` 徽标（hover 提示"全站时间字段均为
  UTC，比北京时间（UTC+8）慢 8 小时"），`Updated At {完整时间戳}` 改成
  `Updated {日期}`——全站只需在一处声明时区基准，不用每个时间戳都标注。

验证：`python -m pytest` 411 passed；Playwright 实测 Overview/Product/Campaign
Detail 抽屉，确认 Data readiness、Comparison coverage 已不可见，Daily Highlights /
Timeline / Detail 面板均只显示日期，顶部徽标可见，0 page error。

### 追加清理：页脚免责声明 + 「口径与排序规则」折叠框（同日）

上面这轮之后用户又指出两处同类"自我说明性"文案也该去掉：每个页面底部的
`.footnote`（"SQLite 为唯一真相源...共 N 个分析批次（真实/模拟）...Zoomex 基线
累计 N 条..."大段方法论说明）、Overview/Campaign/Product/Listing 顶部的
`口径与排序规则`折叠框（`ruleGuide()`，默认折叠但用户认为整个都不该留）。

- 删除 `render()` 里的 `.footnote` 整块（连同 `DATA.meta.insights_total`/
  `insights_mock`/`zoomex_baseline_total` 三个字段在前端的唯一读取点，字段本身
  仍保留在 `dashboard.json` 导出里未删——只是前端不再读，其它消费方要用还在）。
- 删除 `ruleGuide()` 函数本体及其 3 处调用（overview/campaign·product 共用的
  `renderCategoryPane`/listing）。
- 联动清理不再被引用的 CSS：`.rule-guide`/`.rule-guide summary`/
  `.rule-guide-body`/`.rule-guide code`（含暗色主题覆盖）、`.footnote`/
  `.footnote code`/`.mock-note`（含暗色主题覆盖）、以及此前一轮遗留的
  `.readiness-grid`/`.readiness-item`/`.quality-banner`（连同响应式断点里的
  `.readiness-grid` 覆盖）。单条公告上的"模拟"徽标（`mockBadge()`，逐行显示、
  非全局大段说明）不受影响，继续保留。

验证：`pytest tests/dashboard` 29 passed；Playwright 重新截图 Overview 底部、
Campaign/Product/Listing 顶部，确认两处均已消失，0 page error。

## Stage3 业务判断 gap_type 与 reason 自相矛盾：guardrail + 现有数据修复（2026-07-22）

用户在 Detail 抽屉发现一条 Bitunix product 记录（Instant IDR Deposits...）标签是
"未进行对比"，但右侧 ZOOMEX COMPARISON 卡片和下方 diff_detail 都有实质内容，
问"为什么未进行对比但 zmx 内容非空"。

### 根因（两层）

1. **右侧卡片内容跟"是否对比过"本来就是两个独立字段**：卡片文字来自
   `zmx_capability_desc`（Zoomex 能力目录里同 `mechanism_type` 的通用能力说明，
   跟这篇公告有没有被具体比对过无关），"未进行对比"标签来自另一个独立字段
   `diff_type`（Stage3 对**这篇**公告单独判断的结论）——两者字面上不矛盾，是
   预期设计。
2. **但这一条的 `diff_type` 本身是模型自相矛盾的**：`gap_type=baseline_not_found`
   （语义"没召回到候选，不能比较"）+ `zmx_evidence=[]`，但 `reason`/`diff_detail`
   原文却点名了具体候选（"候选 z2 为 Web 端 OnlinePay...同属法币通道但本地化即时
   IDR 机制不同"）——模型自己描述了一次真实比较，却把 gap_type 判成了"没法比较"。
   按 `GAP_TYPE_TO_DIFF_TYPE` 语义，这种"同类但机制不同"的描述本该是
   `different_mechanism`（→"ZMX玩法不同"），不是 `baseline_not_found`。

真实核查：用检测这个模式的脚本扫全库 `campaign`/`product` 类目的
`articles_analysis`，全库仅 `Bitunix/product/EN/2026-07-21` 这一批次命中，共 2 篇
（另一篇是"下架 10 个 Earn 产品"，reason 点名"候选 z4"）。

### Guardrail（防止以后再犯）

- `src/analysis/llm.py::validate_business_judgment()` 新增检查：`gap_type` 判
  `baseline_not_found`/`not_applicable` 且 `zmx_evidence` 为空，但 `reason` 里
  引用了本批次候选序号（正则 `\bz(\d{1,2})\b`，候选序号来自 prompt 自己定义的
  `z1`/`z2`/... 标签）→ 记一条 `issue`（`reason_cites_candidate_but_gap_type_*`），
  不擅自把 gap_type 改写成某个"有候选"的取值——代码无法安全猜出真实该是
  confirmed_gap/different_mechanism/covered 里的哪一个，只能标记供人工/下次重跑
  复核。`run.py` 里 `judgment.issues` 非空现在也会计入 `report.validation_failed`
  并 `logger.warning` 打印，之前只有 JSON 解析失败才会计数，问题会被静默吞掉。
- `src/analysis/prompts.py`：`SYSTEM_BUSINESS_JUDGMENT` 和
  `build_business_judgment_prompt()` 的 user 指令都加了一句显式约束——reason
  点名候选序号就必须在 zmx_evidence 里列出且 gap_type 不能是
  baseline_not_found/not_applicable；反之 baseline_not_found/not_applicable 时
  reason 不能点名具体候选。`prompt_versions.business_judgment` 从 v1 递增到
  v2（config/analysis.yaml），让 Stage3 缓存对全部存量批次失效、下次跑重新走
  LLM（Stage1 事实抽取的缓存不受影响，按 content_hash 算，跟这次改动无关）。
  `tests/analysis/test_llm.py` 新增 2 个用例（自相矛盾的 reason 被 flag、正常的
  "候选都不匹配"泛述不会被误报）；`tests/analysis/test_run.py` 的
  `prompt_version` 断言字符串同步改成 v2。

### 现有数据修复：结果好坏都如实记录

备份主库后，对 3 个历史 `batch_date`（2026-07-21/07-20/07-15）分别真实跑了
`python -m src.analysis --category campaign,product --provider cursor_agent
--max-tokens 400000`（`--source` 覆盖全部 5 个竞品）。07-15 的批次因为对应公告
早已被之后的每日增量重新抓取过（`fetched_at`/`status` 已经翻篇），`get_batch_rows`
按 `date(fetched_at)=batch_date` 查不到东西了，0 命中——这是系统按设计"历史窗口
关闭后不可逆"的正常行为，不是这次改动引入的问题。07-21/07-20 两天共真实处理
15 个批次（13 + 2，其余是同日 EN→FR/VN/ID 复用，非真实调用），总计
`llm_calls=18 total_tokens≈378826`。

**结果：guardrail 按预期在重新计算的输出里再次抓到了同样这 2 条**
（`Stage3 业务判断校验发现问题 Bitunix/product/EN: [...reason_cites_candidate_but_gap_type_baseline_not_found...]`，`validation_failed=1`）——
说明这次 prompt 加固**没有**让模型自我纠正这两条的判断（`temperature=0`，同样的
input 换来几乎同样的 output，符合预期，不是复现失败）。全库重新扫描确认：除这
2 条外，没有发现其它同类矛盾。这 2 条目前诚实地保留原状（`diff_type` 仍是
"不适用"，但现在有 `issue` 记录在案，下次真的换了更强的 prompt/模型再跑时会
被同样的 guardrail 继续盯着），没有代码自动"猜"一个 different_mechanism 出来
覆盖它——那样等于工具替 LLM 编造分析结论。

验证：`pytest` 416 passed；Playwright 重新打开受影响的 Detail 抽屉确认渲染正常、
0 page error；`docs/data/dashboard.json` 已用主库重新导出。

## 追查真正根因：Stage2 词项重叠召回没过滤停用词，Product 几乎全员"未进行对比"（2026-07-22）

上一节以为只有 2 条自相矛盾，用户指出实际是 **Product tab 几乎全部内容**都被判
"未进行对比"——重新核查 `docs/data/dashboard.json`：`product_all` 33 条 100%
`diff_tag=na`；`campaign_all` 58 条里 38 条（65%）也是 na。这不是个别 LLM 判断
失误，是 Stage2 确定性候选召回（`recall_candidates`/`select_relevant_catalog`，
均为词项重叠算法，无 LLM 参与）系统性失效。

### 真实核查

写了一次性诊断脚本，原样复用 `run.py` 的真实调用序列
（`get_catalog_digest` → `select_relevant_catalog` → 逐篇 `recall_candidates`），
对 `Bitunix/product/EN/2026-07-21`（12 篇完全不同主题：tick size 调整、AUSTRAC
注册、SEPA 主体变更、CRWD 拆股合约调整、IDR 钱包、Earn 下架...）实测发现：几乎
每一篇都被召回同样的 `wallet`/`card_payment` 候选，不管文章主题是什么。

根因：`src/analysis/staged.py::_terms()`（`recall_candidates` 用）和
`src/analysis/zmx_catalog.py::_terms()`（`select_relevant_catalog` 用）**都没有
过滤停用词**——`_TOKEN_RE` 只要求长度 ≥2（`staged.py`）/ ≥3（`zmx_catalog.py`）
的字母数字/CJK 序列即算一个"词项"。中文因为 `_TOKEN_RE` 天然滤掉了"的"/"了"这类
单字虚词，这套算法在中文语料上工作正常（现有单测全是中文场景）；但英文的
"is"/"to"/"users"/"trading"/"account"/"platform"/"service"/"support" 这类高频
虚词或行业通用词长度都够格，导致几乎任何两段英文商业文本都能碰出至少 1 个重叠
词——而 `recall_candidates` 的召回门槛只要求 **score>=1**（"零重叠不返回候选"，
反过来就是"有 1 个重叠就返回"），门槛形同虚设。真实结果就是几乎所有英文
Product 公告都被强行"匹配"到 Zoomex 目录里恰好文字比较啰嗦、被停用词碰撞概率
更高的那一两个条目（wallet/card_payment），而不是真正主题相关的条目——这也是
上一节那 2 条"reason 点名候选却判 baseline_not_found"的更深层成因：模型拿到的
"候选"本来就是噪声匹配出来的，模型如实指出"这个候选跟我不是一回事"，只是被
prompt 要求的 gap_type 取值判错了。

### 修复

- `src/analysis/staged.py`：新增 `_STOPWORDS` 冻结集合（纯语法虚词 a/an/and/
  the/is/to/... + 高频无区分度行业套话 users/trading/account/platform/service/
  feature/support/available/new/now/official/launch/live/update 等），`_terms()`
  过滤掉这些词再参与重叠计分。刻意保守：不碰任何可能承载真实语义的具体机制词
  （fee/deposit/tick/wallet/copy/risk 等一律保留）。
- `src/analysis/zmx_catalog.py::_terms()`（`select_relevant_catalog` 批次级候选
  窄化用）同步加了同一份停用词表——这一处输入是整批文章标题+正文拼接（比单篇
  facts 更长、噪声词命中概率更高），不修就只修了后一半漏了前一半。两处独立维护
  （`zmx_catalog.py` 反过来会被 `staged.py` import，放一起会循环 import），改动
  时需要两边一起改，各自注释里做了交叉引用提醒。
- `tests/analysis/test_staged.py` 新增 2 个用例（只共享停用词的两段文本不再被
  判定"相关"；真正共享机制词——如 deposit/tick——的情况仍然正确召回，防止矫枉
  过正）；`tests/analysis/test_zmx_catalog.py` 新增 1 个 `select_relevant_catalog`
  层面的对应用例。`pytest` 420 passed。

### 效果验证（真实重新计算，不是模拟）

改动前用同一份诊断脚本对该批次逐篇打印候选，几乎全部指向 wallet/card_payment；
改动后：tick size 调整 ×2、Fixed Risk ×2、Risk Limits 调整这几篇**正确召回 0
候选**（Zoomex 目录里确实没有对应的技术性/风控类条目，这是符合语义的"真没有"，
不是 bug）；"Earn 产品下架"从原来碰撞出的噪声候选变成精确召回 `earn` 类型
唯一候选。

备份主库后，对 2026-07-21/2026-07-20 两个仍可追溯的历史批次（07-15 的公告
`fetched_at` 已翻篇，同上一节的已知限制）重新跑
`python -m src.analysis --category campaign,product --provider cursor_agent
--max-tokens 400000`，共 15 个真实批次、`llm_calls=12 total_tokens≈257029`。
结果：

- 上一节两条自相矛盾全部消失（`validation_failed=0`）：「Delisting of Certain
  Earn Products」现在正确判定为 `ZMX玩法不同`（真实召回到 Zoomex Saving 产品做
  对照，Detail 抽屉里 Zoomex Comparison 卡片现在有真实的对照文章和链接）；
  「Instant IDR Deposits」保持 `不适用`，但 reason 改成了"召回候选机制类型均与
  IDR 本地即时法币通道不匹配"——不再点名任何候选序号，自洽。
- 全局改善：`product_all` 33 条里 na 从 33 降到 31（2 条转为真实 `diff`）；
  `campaign_all` 58 条里 na 从 38 降到 36（22 条 diff，原来 20 条）。

**如实说明改善幅度有限的原因**：这次修复解决的是"因为停用词噪声被错误召回/
错误比较"这一类问题，不是让 Zoomex 目录凭空多出更多可比条目。Bitunix product
类目本身覆盖大量 Zoomex marketing 向目录里天然没有直接对应物的运营/合规类公告
（tick size 调整、SEPA 主体变更、AUSTRAC 注册、CRWD 因股票拆股调整合约规模等），
这些**修复后正确召回 0 候选**、合理停留在"不适用"，属于真实的类目结构差异，
不是还没修好的 bug。也就是说"Product tab 大多数是未进行对比"这个现象本身部分
是真实业务情况（不是所有竞品技术更新都有 Zoomex 对应功能），这次修复的价值是让
"未进行对比"里不再混杂"被停用词误导致产生自相矛盾结论"的假阳性，而不是消灭
"未进行对比"这个状态本身。

验证：`pytest` 420 passed；Playwright 重新打开修复后的 Delisting/IDR 两条 Detail
抽屉，确认渲染正常、0 page error；`docs/data/dashboard.json` 已重新导出。

## "未进行对比"跟右侧有内容的视觉矛盾：两套独立信号没有互相感知（2026-07-22）

上一节修完召回噪声后，用户又指出 Lbank「P2P Limited-Time Offer for New Users」
这条还是"右侧 Zoomex 卡片有内容，标签却是未进行对比"。追查发现这次不是召回噪声，
是**两套完全独立的信号从设计上就没打通**：

- 右侧卡片内容来自 `zmx_capability_desc`——`export_data.py` 按这篇公告自己的
  `mechanism_type` 标签（Stage1 打的，如 `zero_risk_new_user`）去 Zoomex 目录
  ROLLUP 表（`zmx_catalog_entry`，覆盖 Zoomex 全量历史）查"这个标签 Zoomex 是否
  出现过"，是**标签相等**的粗粒度检查。
- 下面的"未进行对比"标签来自 `diff_type`——Stage3 对**这一篇**逐篇比对的结论，
  只在当批次窄召回的候选（`zmx_summary` 具体条目，不是标签）里找，没找到判
  「不适用」。

这篇 Lbank 公告读原文实际是"首次 P2P 购买超低价"，跟 taxonomy 里
`zero_risk_new_user`（"首单亏损包赔"类机制）字面对上号但语义不完全一致——Stage1
分类没有非常精确，但用户判断这种粗粒度打标"收益递减、不值得再抠"，问题不在分类
准不准，而在**前端把两套信号当成一回事展示，制造了假象**：粗粒度的"这个类型
Zoomex 有"跟逐篇比对的"没找到具体对应"同时出现时，只显示后者会让人以为"有内容
却没比过"自相矛盾。

### 修复（纯展示层，不改任何已存的分析数据）

- `src/dashboard/export_data.py::build_category_section()`：`diff_tag='na'` 且
  `zmx_exists=='yes'`（**精确**标签命中，不是 `'partial'` 近似匹配——那个本身就
  标注"建议人工核核实"，置信度不够）且没有具体 `zmx_counterpart` 时，把展示用的
  `diff_tag` 降级改成新值 `'broad'`；`diff_type`/`diff_detail` 原样保留 Stage3
  自己的逐篇结论，不擅自改写，只是多了一层展示层面的"我们还知道点别的"信号。
  `DIFF_TYPE_SORT_ORDER` 新增 `broad: 3`（跟 `same` 同档，`na` 单独降到 4）。
- `docs/index.html`：新增 `broad` → `已有同类型 · 粗粒度` 标签 + 独立配色
  （蓝色，跟"已有同类"的绿色、"na"的灰色都区分开）。`zmxCompareTwoColumn()`
  的 Detail 抽屉里，`diff_tag==='broad'` 时右侧卡片顶部加一行明确提示"同类型
  参考 · 粗粒度匹配，非逐篇核对"，下方标签后的说明文字加"具体比对："前缀——
  让读者一眼看出"上面这段是粗粒度参考，下面这句才是这篇文章真正的逐篇比对
  结论"，两个信号不再互相打架。表格里的 `zmxComparisonBox()`/Markets 详情等
  其它复用同一份 `DIFF_LABEL` 的展示点自动跟着生效，不需要逐个改。
- `tests/dashboard/test_export_data.py` 新增 2 个用例：`exists_flag='yes'` 时
  na 正确降级为 broad 且原始字段不变；`exists_flag='partial'` 时不触发降级
  （防止把弱信号也当成强信号处理）。`pytest` 422 passed。

### 效果（纯导出层重算，没有动 LLM/主库分析数据）

`docs/data/dashboard.json` 重新导出后：`product_all` 33 条里 na 从 31 降到 24
（7 条变成 `broad`）；`campaign_all` 58 条里 na 从 36 降到 23（13 条变成
`broad`，`diff` 22 条不受影响）。Lbank 这条现在 Detail 抽屉里读起来是"Zoomex
大致有这类新用户零风险玩法（粗粒度参考）；具体比对：候选均不匹配，Lbank 这个
P2P 折扣机制跟 Zoomex 那个亏损包赔机制不是一回事"——两句话不再互相矛盾。

验证：`pytest` 422 passed；Playwright 打开该 Detail 抽屉截图确认新样式渲染正常、
0 page error。

## 粗粒度匹配补链接 + 差异筛选器收敛到 4 类（同日）

用户看完 `broad` 标签的效果后提两个后续需求：① 粗粒度匹配的 Zoomex 内容也要能
点进去看原文，不能只有文字；② Campaign/Product/Search 三处"全部差异"筛选器只
保留【未检索到同类、已有同类·玩法不同、已有同类·粗粒度、未进行对比】四类
（`same`/`mixed` 目前真实数据里从未出现——`same` 需要 Stage3 找到具体证据且判
完全一致，`mixed` 只在批次级 `insights.diff_type` 出现、不会落到逐条
`diff_tag` 上，`pending` 本来就不在筛选器里）。

- `src/dashboard/export_data.py::_load_zmx_catalog_index()`：新增读取
  `zmx_catalog_entry.example_uids`（rollup 时最多存 3 条同 mechanism_type 的
  Zoomex 公告 uid），取第一条 join `announcements` 拿真实 url，新字段
  `zmx_capability_url` 挂到 campaign/product 行上。只读现有数据，不需要重新跑
  LLM。
- `docs/index.html`：`zmxCompareTwoColumn()` 的 `broad` 分支、`zmxComparisonBox()`
  （表格紧凑列）都加了"查看同类型示例 ↗"/`Open Zoomex evidence ↗` 链接
  （有 `zmx_capability_url` 才显示）。新增 `DIFF_FILTER_TAGS = ['missing',
  'diff', 'broad', 'na']` 常量，Campaign/Product 的"全部差异"下拉、Search 的
  差异筛选按钮组三处都改用这个常量生成选项，不再用
  `Object.entries(DIFF_LABEL)` 遍历全部——`DIFF_LABEL` 本身不删 `same`/`mixed`
  这两个 key（万一真出现这两种 diff_tag，表格里仍然能正确显示标签文字，只是
  筛选器里选不到）。
- `tests/dashboard/test_export_data.py`：`_zmx_catalog_entry()` 测试 helper 加
  `example_uids` 参数；`test_diff_tag_downgrades_na_to_broad_...` 用例扩展，
  断言 `zmx_capability_url` 能正确解析到 Zoomex 示例文章的真实 url。`pytest`
  422 passed。

验证：Playwright 打开 Lbank P2P 那条 Detail 抽屉，右侧卡片新增"查看同类型示例"
链接可点击、指向真实 Zoomex 文章 URL；Campaign/Product/Search 三处筛选器
`eval_on_selector_all` 确认选项精确是 `['all','missing','diff','broad','na']`，
0 page error。

## 新增文档：docs/pipeline.md（同日）

用户要求整理一份完整的处理/分析逻辑流程文档，不同于 `docs/history.md`（按时间
顺序的调试日志）和 `CLAUDE.md`（现状快照），新增 `docs/pipeline.md` 按**数据流
顺序**讲解全链路：采集（watermark/full_scan）→ Pipeline 后处理（group-check/
dedup/classify/region）→ Zoomex 能力目录（extract/rollup，exists_flag 三态）→
竞品 LLM 分析 staged-v1（Stage1 事实抽取/Stage2 确定性召回/Stage3 业务判断，
含 2026-07-22 停用词教训）→ 批次落库与 locale 复用 → Dashboard 导出（含
diff_tag 完整语义速查表，覆盖 missing/diff/same/mixed/broad/na/pending 七种
取值）→ 前端展示 → 飞书同步/推送，末尾附"关键设计原则"和"已知的粗糙之处"两节。
写作时核对了 `config/zmx_mechanism_taxonomy.yaml` 真实条目数（product 是 10 个
+ other，不是凭印象的数字）。

## "未进行对比"/"尚未分析"大量残留的真正根因：历史遗留批次从未被现行管线覆盖（2026-07-22）

用户看完 `broad` 效果后指出更大的问题：看板存量数据里"未进行对比"/"尚未分析"
数量仍然很多，认为"未进行对比"本该只出现在 listing/other 类，Product 等类目
"没匹配到"应该显示"未检索到同类"。

### 真实核查：两条独立病灶，不是同一个 bug

1. **`na` 但 `zmx_exists=None`（不是 `'no'`）占绝大多数**：抽查发现这些行的
   `mechanism_type` 要么是 `null`，要么是"邀请/推荐""空投/Launch""积分活动"这类
   **自由文本中文标签**——不是 `config/zmx_mechanism_taxonomy.yaml` 定义的封闭
   枚举值。查 `insights.prompt_version` 实锤：这些行是 `campaign-v1`/`campaign-v3`/
   `product-v1`/`product-v3`（Phase②`staged-v1` 上线前的旧监体/中间版本
   prompt），根本没跑过现在这套 Stage1/2/3 管线，`mechanism_type` 字段形状
   完全对不上，导出层只能优雅降级成 `na`。
2. **`comparison_status='pending'` 的行（11 campaign + 6 product）**：这些
   公告的 `post_time` 早到 2026-06-15～2026-07-16，但从未出现在任何一次
   `insights` 批次里——查 `announcements` 发现它们的 `fetched_at` 已经全部被
   之后的每日增量刷新到 2026-07-21/07-20（`status` 也早已变回 `unchanged`）。
   `run.py::get_batch_rows()` 的查询条件是 `status IN ('new','changed') AND
   date(fetched_at)=batch_date`——这批公告要么当天 daily 分析没跑（运维缺口），
   要么跑过但预算熔断跳过了，之后再也没人专门去补，从此永远卡在两头不着：
   按原历史日期查不到（`fetched_at` 已翻篇），按今天日期查也查不到
   （`status` 不是 new/changed 了）。**这是 `docs/history.md` 早前"07-15 批次
   fetched_at 已翻篇查不到"那条已知限制的更严重版本**——当时以为只影响一天，
   实际上因为常规 daily 分析从不主动补历史缺口，缺口会一直累积。

### 修复

- **`src/analysis/run.py` 新增 `--include-unchanged` CLI 参数**（`run()` 函数
  参数早就有，只是没接到 argparse）：连 `status=unchanged` 的公告也纳入批次，
  专门解决"当天分析没跑、之后 fetched_at 已经滚到今天"这类历史遗留公告永远
  查不到的问题。
- **`src/dashboard/export_data.py` 新增 `na → missing` 的对称升级逻辑**（跟
  之前 `na → broad` 是同一处代码、同一个设计思路的镜像）：`diff_tag='na'` 且
  `zmx_exists=='no'`（rollup 覆盖 Zoomex 全量历史后**确认**这个 mechanism_type
  从未出现过，不是"没查"）时，展示用 `diff_tag` 升级成 `'missing'`
  （未检索到同类）；`exists_flag` 为 `None`（没打标签/没有对应目录条目）时
  维持 `'na'`，那才是真正"不知道"的情况，不能升级。`diff_type`/`diff_detail`
  原始 Stage3 结论不改写。新增 2 个测试（`exists_flag='no'` 触发升级 +
  `mechanism_type=None` 时不触发）。
- **真实数据修复**：备份主库后，用新 flag 真实重跑
  `python -m src.analysis --date 2026-07-21 --category campaign,product
  --source Bitunix,Weex,BingX,Phemex,Lbank --include-unchanged --provider
  cursor_agent`——批次范围从"只看 new/changed"扩大到"当天存在的全部
  campaign/product 公告"，单批次文章数从个位数暴涨到 25/24/18 篇不等。分两次
  跑完（第一次 `--max-tokens 900000` 熔断在跑到一半，`cursor_agent` 后端真实
  单次调用 token 消耗比预估高得多——45 次调用就吃掉 90 万 token；第二次
  `--max-tokens 3000000` 补完剩余批次，Stage1 缓存复用了第一次的进度，没有
  浪费），共 20 个批次全部真实分析完成（`analyzed=15 derived=5`，无跳过），
  期间 1 篇文章 Stage1 JSON 解析失败（如实计入 `validation_failed`，不是
  静默吞掉，该文章走 `unknown` 兜底）。
- **清理孤儿数据**：18 条旧格式 insights 行（`campaign-v1/v3`、`product-v1/v3`）
  逐一核实其 `related_uids` 是否已完全被新的 2026-07-21 批次覆盖（全部确认
  是真子集关系），征得用户确认后删除——不删的话 `_load_article_index()` 按
  uid 建索引时同一篇公告会有新旧两条格式不同的记录，哪条胜出取决于无
  `ORDER BY` 的表扫描顺序，属于隐患。删除前单独备份一次主库。

### 效果（真实重新导出，不是模拟）

`comparison_status='pending'` 从 campaign 11/product 6 降到 **两边都是 0**；
`diff_tag='na'` 从 campaign 23→5、product 24→13；`campaign_all` 首次出现
`missing`（未检索到同类，1 条）；`product_all` 首次出现 `same`（已有同类，
3 条）——这两个标签此前在真实数据里从未出现过，现在随着历史缺口被真实分析
覆盖，两者都是货真价实的新结论，不是造假凑出来的。

验证：`pytest` 431 passed；Playwright 打开 Product/Campaign 两个 tab 截图确认
渲染正常、0 page error；`docs/data/dashboard.json` 已重新导出。
