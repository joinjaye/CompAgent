...
| 1 | 数据源侦察 | 填满的 sources.yaml + 每个源一份真实响应 fixture | 进行中 |
...
以下表格随每个源侦察完成实时更新，详细侦察记录见 `config/sources.yaml` 对应条目上方注释，
真实响应快照见 `tests/fixtures/`。

| 交易所 | locale | 状态 | 策略 | 备注 |
|---|---|---|---|---|
| Zoomex（我方基线） | EN / FR / EN-Asia / VN / ID | ❌ 受阻 | 待定 | 纯客户端渲染 Nuxt 3 SPA，服务端只返回通用壳（5 个 locale md5 一致），静态分析 263 个 JS chunk 未能还原公告 API；已确认 API host 为 `api2.zoomex.com` 但真实 path 未知。建议：headless browser 或内部要 API 文档。详见 `config/sources.yaml` zoomex 块注释 |
| Bitunix | EN / FR / ID | 待开始 | — | — |
...
| BingX | EN | ✅ 通 | full_scan | Nuxt 3 SSR，`__NUXT_DATA__` 内嵌数据；首屏仅~20条，完整历史用 sitemap（7829 URL）替代；createTime==updateTime 恒等 |
| BingX | VN | ✅ 通 | full_scan | 同 EN 机制，article_id 跨 locale 一致可做 group_id；VN sitemap 不完整需借用 EN sitemap |
| Phemex | EN | ✅ 通 | full_scan | SSR，`window.preloadedData` 内嵌数据，detail 页 inline 正文；updatedAt 只是秒级发布噪音；sitemap_Announcement.xml 给全量文章+跨语言映射 |
| Phemex | FR | ✅ 通 | full_scan | 同 EN 机制 |
| Lbank | EN | ✅ 通 | watermark | Next.js SSR，正文内嵌在列表页 RSC JSON 里；updateTime 需另请求 detail 页（/support/articles/{code}）。仅第 1 页（10条）可稳定拿到，翻页未逆向 |
