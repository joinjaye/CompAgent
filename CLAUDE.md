...
│   │   │                        #   2026-07-21 起 fetch_detail() 空正文兜底用标题（isRedirect
│   │   │                        #   跳转型文章 / Slate.js 纯图片文章，真实内容抓不到，见
│   │   │                        #   函数内注释）；2026-07-22 起 normalize() 填充 url（此前
│   │   │                        #   一直是 None，见「Zoomex 详情页 URL」小节）
...
│   │   └── dedup.py             # ✅ 2026-07-21 新增：同源同 locale「标题+正文完全一致」重复公告
│   │                              #   检测，标记 announcements.duplicate_of（不是只按 content_hash——
│   │                              #   见文件顶部说明，源站会有不同事件复用模板正文、标题不同的假重复）
...
│   ├── pipeline/
│   │   └── test_dedup.py         # ✅ 2026-07-21 新增：dedup.py 单测（真重复 vs 假重复/空内容不聚类）
│   ├── test_db.py               # db 层单测（Phase 0）
│   ├── test_migrate_v2.py        # migrate_v2.py 单测（Phase 2.5）
│   └── test_migrate_v3.py        # migrate_v3.py 单测（Phase 4，migrate_v4.py 逻辑简单，
│                                  #   幂等 ALTER TABLE ADD COLUMN，未单开测试文件）
├── data/
...
└── scripts/
    ...
    ├── migrate_v4.py             # ✅ 2026-07-21 新增：schema v3 -> v4 迁移：announcements 新增
    │                              #   duplicate_of 列（幂等 ALTER TABLE ADD COLUMN）
...
| duplicate_of | TEXT | ✅ 2026-07-21 新增（schema v4，`scripts/migrate_v4.py`）。同源同 locale 下标题+
  正文完全一致的重复公告，指向被判定为"更早"一条的 `uid`；NULL 表示本行是规范行或未参与
  去重判断。由 `python -m src.pipeline dedup --apply` 写入（判重口径见 `src/pipeline/dedup.py`
  顶部说明：不能只按 `content_hash`，源站会有不同事件复用模板正文、标题不同的假重复）。
  `src/analysis/batch.py`/`src/analysis/zmx_catalog.py` 的批次查询、`src/dashboard/export_data.py`
  的看板内容列表（campaign/product/listing/delisting 卡片、trend、markets、search_index）都排除
  `duplicate_of IS NOT NULL` 的行；`source_coverage` 的采集量统计（今日/累计条数）故意不排除，
  仍反映真实抓取量 |
...
- **飞书群日报已收敛到 EN 群**：应用机器人将三份 LLM Summary、四个业务入口与
  显式选择“最新批次”后的 Overview 截图合并为一张交互卡片并发送；不使用业务群
  webhook。图片失败时降级发送纯文本卡片。
- **Zoomex `zmx_baseline.mechanism_type` 标签碎片化：已解决（2026-07-21）**——
  ...
  staged.py 竞品分析管线 + Dashboard 事件级 Detail」小节）。**主库已迁移到新
  schema 并已真实运行过**（更新于 2026-07-21）——`data/competitor_intel.db` 已有
  `zmx_summary`（591 行）/`zmx_catalog_entry`（28 行）两张表，`python -m
  src.analysis.zmx_catalog extract`/`rollup` 已在主库上真实跑过多次，不再是
  scratch db 专属验证。
...
- **没有真实的 OpenAI 兼容 LLM 后端**：`.env` 的 `LLM_API_KEY`/`LLM_API_BASE` 仍为空，
  ...
  真实 OpenAI 兼容响应格式/行为从未验证过。**该后端偶发不稳定**（2026-07-21 观察到
  三种不同失败形态：整段时间窗口内 100% 失败/`status=error` 无详情、`CloseAgent`
  连接被拒、`InternalServerError`），排查方式是绕过 pipeline 直接跑一个最简单的
  `Agent.prompt("请只回复 OK", ...)` 判断是不是后端本身的问题；确认是后端问题后
  重跑通常就能恢复，`run.py` 对这类异常不重试，是直接整个调用失败（`--max-calls`
  熔断跟这个是两回事，不要混淆：熔断是主动停止，不是失败）。跑批量分析时用高
  `--max-calls`（如 30-50）+ `--max-tokens` 兜底，不要用低 `--max-calls`（那是
  给"验证改动是否生效、控制真实花费"的场景用的，两种场景不要混），见
  `feedback_llm_analysis_max_calls_vs_tokens` 记忆条目。
...
- **同源同 locale 重复公告：已解决（2026-07-21）**——真实核查发现源站会对同一条
  通知换新 `article_id` 重新发布（如 BingX 一条维护保证金率公告相隔约 2 小时出现
  两个不同 ID），旧 schema 无法识别，下游会当成两条独立事件重复计入。新增
  `announcements.duplicate_of` 列（schema v4）+ `python -m src.pipeline dedup
  --apply`，判重口径固定为「同 source+locale+title+content」而不是只看
  `content_hash`——真实数据里发现 Zoomex 自己的 CMS 会让不同事件（如两次不同代币
  的合约上线公告）复用同一段模板正文、只有标题不同，只按 content_hash 判重会把这
  两条完全不同的竞品情报误判成重复并合并丢失。已在主库跑过一次 `dedup --apply`：
  13 组重复、14 行标记。`src/analysis/batch.py`/`zmx_catalog.py`/
  `export_data.py` 均已排除 `duplicate_of IS NOT NULL` 的行，`run_daily_pre_lark.sh`
  已加入这一步（group-check/classify/region 之后，ZMX catalog 之前）。
- **Zoomex 空正文：已解决（2026-07-21）**——真实核查（`curl getArticleById`
  逐条验证）发现 5 篇文章（19 行，覆盖多 locale）正文为空，是两类真实情况，不是
  解析 bug：(1) `isRedirect=true` 跳转型文章，真实内容在 `redirectUrl` 落地页
  （该页是 help.zoomex.com 同款纯客户端渲染 SPA 壳，抓不到真实文案，未构建
  scraper，投入产出比不划算）；(2) Slate.js 内容只有图片节点、没有文字（如新年
  祝福图）。`src/collectors/zoomex.py::fetch_detail()` 现在对空正文兜底用标题，
  已用一次性脚本对主库这 5 篇文章（19 行）重新抓取详情并落库，并重跑
  `zmx_catalog extract`/`rollup` 让其中的 campaign 内容（4 条）进入目录，不再对
  Stage2 召回不可见。
...
- 飞书群推送：Phase 6 规划的"逐条规则推送引擎"从未实现；现有日报是“LLM Summary
  交互卡片 + Overview 截图”（`src/dashboard/screenshot.py` +
  `src/sinks/feishu_bot.py`），只发送到 EN 群。
## Dashboard PDF 缺口本地优化（2026-07-21）

按《可视化呈现调整》PDF 对本地 category-first 看板补齐一期可由现有真实数据支撑的部分，
未发布线上版本：

- 导出新增 `product_all` / `listing_all` / `announcements_all`，与已有 `campaign_all`
  一起供静态前端做发布时间范围筛选；最新批次字段保持不变。
- 顶栏新增 From/To 与“最新批次”恢复按钮。Campaign 的日期匹配同时考虑活动生命周期
  相交，避免正在进行的旧活动因发布时间不在范围内被误删。
- Campaign 新增独立 Campaign Update 区，以及奖励信息、市场/语言筛选；Detail 补充
  Announcement Time、参与机制和已有奖励字段。
- Product Summary 改为 New / Updated / Affected Exchange / Affected Category，列表明确
  分成 New Product 与 Product Update，并按 New → Feature Added → Feature Removed →
  Rule → UI → Performance → Operational Notice 确定性排序。
- 没有伪造 Product Entity / Change History、页面级 AI Insight 或源数据未抽取出的奖励细项；
  这些仍是数据模型/分析管线后续项。

验证：`tests/dashboard` 28 passed；真实 Chromium 检查 1440px/375px，6 个一级 tab，
自定义日期范围、Campaign Update、Product 分区均无 page error，页面级横向溢出为 0。
